resource "azurerm_kubernetes_cluster_extension" "extension_1" {
  name              = "backup-extension"
  cluster_id        = azurerm_kubernetes_cluster.aks_1.id
...
resource "azurerm_role_assignment" "extension_1_storage_account_contributor" {
  ...
  principal_id         = azurerm_kubernetes_cluster_extension.extension_1.aks_assigned_identity[0].principal_id
}
