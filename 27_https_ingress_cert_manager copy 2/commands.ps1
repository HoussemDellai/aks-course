...
az keyvault create -n $AKV_NAME -g rg-aks-cluster-we
...
az aks enable-addons --addons azure-keyvault-secrets-provider -n aks-cluster -g rg-aks-cluster-we
...
az aks show -g rg-aks-cluster-we -n aks-cluster --query addonProfiles.azureKeyvaultSecretsProvider.identity.clientId -o tsv
# 47744279-8b5e-4c77-9102-7c6c1874587a

az identity create -g rg-aks-cluster-we -n keyvault-identity 
# {
...
$identity_id=$(az identity show -g rg-aks-cluster-we -n keyvault-identity --query "id")
echo $identity_id
...
kubectl describe secret ingress-tls-csi -n $NAMESPACE
...
kubectl apply -f test-kv-pod.yaml -n $NAMESPACE
...
kubectl exec nginx-secrets-store -n $NAMESPACE -- ls /mnt/secrets-store/aks-ingress-cert
# /mnt/secrets-store/aks-ingress-cert

kubectl exec nginx-secrets-store -n $NAMESPACE -- cat /mnt/secrets-store/aks-ingress-cert
# -----BEGIN PRIVATE KEY-----
...
az aks addon update -n aks-cluster -g rg-aks-cluster-we -a azure-keyvault-secrets-provider --enable-secret-rotation --rotation-poll-interval 2m
