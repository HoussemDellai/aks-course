...
resource "azurerm_linux_virtual_machine" "vm-linux-hub" {
  name                            = "vm-linux-jumpbox-hub"
  resource_group_name             = azurerm_resource_group.rg-hub.name
  location                        = azurerm_resource_group.rg-hub.location
...
resource "azurerm_role_assignment" "vm-contributor-hub" {
  ...
  principal_id         = azurerm_linux_virtual_machine.vm-linux-hub.identity[0].principal_id
}
